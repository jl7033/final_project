...
```{r}
lm_batting = lm(log10(salary_2022) ~ ops_2 + service_time_floor,
                 data = merged_batting)
...
lm_pitching = lm(log10(salary_2022) ~ hand + pitcher_type + hand*pitcher_type + ip_total + era_2 + bb9 + so9 + service_time_floor,
...
merged_pitching |>
  ...
  arrange(resid) |>
  select(name, salary_2022, service_time_floor, era_2, pred, resid ) |>
  mutate(expected_salary = 10^pred) |>
  slice(1:5)
